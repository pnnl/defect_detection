...
class RipleysKEstimator_univariate:
    """
    ...
    def area(self, value):
        if isinstance(value, (float, int)) and value > 0:
            ...
            raise ValueError(
                f'area is expected to be a positive number. Got {value}.')
    ...
    def y_min(self, value):
        if value is None or isinstance(value, (float, int)):
            ...
            raise ValueError(
                f'y_min is expected to be a real number. Got {value}.')
    ...
    def x_min(self, value):
        if value is None or isinstance(value, (float, int)):
            ...
            raise ValueError(
                f'x_min is expected to be a real number. Got {value}.')
    ...
    def _pairwise_diffs(self, data1):
        npts_data1 = len(data1)
        npts_data2 = len(data1) - 1
        diff = np.zeros(shape=(npts_data1 * npts_data2, 2), dtype=np.double)
        k = 0
        for i in range(npts_data1):
            size = npts_data2
            data1_removed = np.append(data1[:i], data1[i+1:], axis=0)
            diff[k:k + size] = abs(data1[i] - data1_removed)
            k += size
    ...
    def Lfunction(self, data1, radii, mode='none'):
        """
        Evaluates the L function at ``radii``. For parameter description
        see ``evaluate`` method.
        """
        return np.sqrt(self.evaluate(data1, radii, mode=mode) / np.pi)

    def Hfunction(self, data1, radii, mode='none'):
        """
        Evaluates the H function at ``radii``. For parameter description
        see ``evaluate`` method.
        """
        return self.Lfunction(data1,  radii, mode=mode) - radii

    def evaluate(self, data1, radii, mode='ripley'):
        """
        ...
        if mode == 'ripley':
            ...
            for k in range(npts_data1):
                ...
                corner = sum(int(abs(i) < np.finfo(np.float32).eps)
                             for i in [dL, dR, dD, dU]) >= 2
            ...
            #normally(n*(n-1)), but already npts_data2 is already npts_data1-1
...
def MCconfidence_univariate(r, mins, maxs, n1,  alpha=0.05, N=99, mode='ripley'):
    ...
    Kest = RipleysKEstimator_univariate(area=np.prod(maxs-mins),
    ...
    for i in range(N):
        ...
        HMC[:, i] = Kest.Hfunction(data1=X, radii=r, mode=mode)
