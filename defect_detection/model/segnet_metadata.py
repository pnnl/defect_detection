...
from https://github.com/Sayan98/pytorch-segnet/tree/master/src
"""
...
class SegNetMeta(nn.Module):
    def __init__(self, channels, nclass, full=False,
                 pretrained=True):
        super(SegNetMeta, self).__init__()
        ...
        self.linear_64= nn.Sequential(*[nn.Linear(15, 100),
                                                nn.Linear(100, 64)
                                                ])

        #accept one more input from the meta-data
        self.encoder_conv_meta = nn.Sequential(*[
    ...
    def forward(self, input_img, meta_input):
        """
        ...
        #input back to original output
        x_40 = F.relu(self.encoder_conv_40(x_3))
        x_41 = F.relu(self.encoder_conv_41(x_40))
        x_42 = F.relu(self.encoder_conv_42(x_41))

        meta_8_8 = F.relu(self.linear_64(meta_input))
        # reshape linear output into additional layer 8 by 8 to add along conv axis
        # add two dimensions to the end
        meta_8_8_axis = meta_8_8.unsqueeze(dim=-1).unsqueeze(dim=-1)
        meta_8_8_repeat = meta_8_8_axis.repeat(1, 1, dim_4[2], dim_4[3])
        # add to convolution axis so it has 1,  512, 8, 8  + 1, 64, 8, 8 instead of 512
        x_meta_8_8 = torch.concat((x_42, meta_8_8_repeat), 1)
        x_meta = F.relu(self.encoder_conv_meta(x_meta_8_8))
        meta_dim = x_meta.size()
        


        x_4, indices_4 = F.max_pool2d(x_meta, kernel_size=2, stride=2, return_indices=True)
        ...
        if DEBUG: # pragma: no cover
            ...
            print("meta_dim: {}".format(meta_dim))
