...
class TopoLoss(nn.Module):
    ...
    def forward(self, output, target, i_epoch):
        # output and target are in format necessary for nn.modules.loss.CrossEntropyLoss()
        #input output is  raw, unnormalized scores for each class
        #target Class indices in the range [0, C)[0,C) where CC is the number of classes
        #code from https://github.com/HuXiaoling/TopoLoss
        loss = nn.modules.loss.CrossEntropyLoss(self.weights)

        value_loss = loss(output, target)
        if i_epoch < 475:
            return value_loss
        else:
            lh = F.softmax(output, dim = 1) #.detach().numpy()
            gt = torch.where(target == 1, 1, 0)
            ...
            for i in range(batch_size):
                lh_current = lh[i,1,:,:]
                gt_current = gt[i,:,:]
                topo_loss_i = getTopoLoss(lh_current, gt_current, topo_size=128)
                topo_loss += topo_loss_i/batch_size
            loss = .99*value_loss + .01*topo_loss
